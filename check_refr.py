...
from transforms import transform_l_to_g, transform_g_to_l
...
if __name__ == "__main__":  
    ...
    for idx, rx in enumerate(rxs):
        ...
        if VALIDATE_REFLECTION:
            print(f'Rx:{rx.point}   Incident Field : {np.linalg.norm(ifield)} , Reflected Field : {np.linalg.norm(ref_field)}')
        else:
            print(f'Rx:{rx.point}   Incident Field : {np.linalg.norm(ifield)} , Reflected Field with diffraction : {np.linalg.norm(ref_field)}')
